...
?>
    ...
    <form method="post" action="{{route('BN_profile_edit_action')}}" enctype="multipart/form-data" >
        @csrf
                    ...
                    <div class="mt-3">
                        <div class="sm:grid grid-cols-1 gap-1">
                            <div class="">
                                ...
                                <input type="text" class="form-control w-full" name="name" autocomplete="off" value="{{$myuser->name}}" />
                            </div>
                        </div>
                        <div class="sm:grid grid-cols-1 gap-1 mt-5">
                            <div class="">
                                <label for="" class="form-label">รายละเอียด</label>
                                <input type="text" class="form-control w-full" name="email"  autocomplete="off" value="{{$myuser->email}}" readonly  />
                            </div>
                        </div>
                        <div class="sm:grid grid-cols-1 gap-1 mt-5">
                            <div class="">
                                <label for="" class="form-label">รูปภาพ</label>
                                <input type="file" class="form-control w-full" name="photo" />
                            </div>
                        </div>
                        @if(isset($myuser->photo))
                        <div class="sm:grid grid-cols-1 gap-1 mt-5">
                            <div class="">
                                <label for="" class="form-label">รูปภาพปัจจุบัน</label>
                                <image width="150" src="{{asset($myuser->photo)}}">
                            </div>
                        </div>
                        @endif
